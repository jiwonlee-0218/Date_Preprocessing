...
class GMPool_spatial_attention_FC(nn.Module):
    def __init__(self, time, hidden_dim, spa_hidden_dim):
        ...
        self.mlp = nn.Sequential(nn.Linear(spa_hidden_dim, hidden_dim), nn.BatchNorm1d(hidden_dim), nn.ReLU(),
...
class GMNetwork(nn.Module):
    ...
    def __init__(self, roi, hidden_dim, spa_hidden_dim, time, num_classes, dropout=0.2):
        ...
        self.gm_spatial_model = GMPool_spatial_attention_FC(time, hidden_dim, spa_hidden_dim)
    ...
    def forward(self, inputs):
        fc_attention = {'temporal-result': [], 'spatial-result': []}


        temporal_result = self.gm_temporal_model(inputs)  ## model -> torch.Size([8, 10, 176, 116])
        inputs = rearrange(inputs, 'b t c -> b c t')
        spatio_result = self.gm_spatial_model(inputs) ## model -> torch.Size([8, 10, 116, 176])

        tem_out = self.temporal_conv1(temporal_result).squeeze(-1)  # model -> torch.Size([8, 32, 176, 1]) -> torch.Size([8, 32, 176])
        spa_out = self.spatial_conv1(spatio_result).squeeze(-1) # model -> torch.Size([8, 32, 116, 1]) -> torch.Size([8, 32, 116])
        ...
        for s in range(spa_out.shape[0]):
            ...
            fc_attention['spatial-result'].append(a)
        ...
        spa_out = self.spatial_conv2(spa_out).squeeze(1) # model -> torch.Size([8, 1, 32]) -> torch.Size([8, 32, 116])


        spatio_temporal_input = spa_out + tem_out # torch.Size([8, 32])
