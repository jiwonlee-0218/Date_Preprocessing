...
class ModuleTransformer(nn.Module):
    def __init__(self, n_region, hidden_dim, num_heads, dropout=0.1):
        super().__init__()
        self.multihead_attn = nn.MultiheadAttention(n_region,
                                                    num_heads)  # (in_features=128, out_features=128, bias=True)
...
class BrainDecoder(nn.Module):
    def __init__(self, n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums, window_size,
                 dropout=0.5):
        super().__init__()

        self.num_classes = num_classes
        self.sparsity = sparsity
        self.subtask_nums = subtask_nums
    ...
    def forward(self, dyn_t, dyn_a, e_s, X_enc, sampling_endpoints):
        ...
        for layer, (G, T, L) in enumerate(zip(self.gnn_layers, self.transformer_modules, self.linear_layers)):
            ...
            q_s = repeat(e_s, 'b c -> t b c', t=num_timepoints) + X_enc[
                [p - 1 for p in sampling_endpoints]]  # torch.Size([34, 2, 128])
...
class BrainNetFormer(nn.Module):
    def __init__(self, n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums, window_size,
        ...
        self.decoder = BrainDecoder(n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums,
                                    window_size, dropout)
        self.subtask_predictor = nn.Linear(hidden_dim, subtask_nums)
    ...
    def forward(self, dyn_t, dyn_a, t, a, sampling_endpoints):
        ...
        # subtask_logit = self.subtask_predictor(rearrange(X_enc, 't b c -> (b t) c'))

        # return logit, subtask_logit, time_attention  # main task 결과, subtask 결과, time attention
        return logit, time_attention
