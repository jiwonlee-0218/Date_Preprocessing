...
####################### important self.subtask_label_list #################################
class DatasetHCPTask(Dataset):
    def __init__(self, sourcedir, subtask_type='detail', roi='aal', dynamic_length=None, k_fold=None, smoothing_fwhm=None):
        ...
        if os.path.isfile(os.path.join(sourcedir, f'{self.filename}.pth')):
            self.timeseries_list, self.label_list = load(os.path.join(sourcedir, f'{self.filename}.pth'))
        else:
            # roi_masker = NiftiLabelsMasker(load_img(self.roi['maps']))
...
class DatasetHCPTask_test(Dataset):
    def __init__(self, sourcedir, subtask_type='detail', roi='aal', dynamic_length=None, k_fold=None,
                 smoothing_fwhm=None):
        super().__init__()
        ...
        task_timepoints = {'EMOTION': 176, 'GAMBLING': 253, 'LANGUAGE': 316, 'MOTOR': 284, 'RELATIONAL': 232,
                           'SOCIAL': 274, 'WM': 405}
        self.sourcedir = sourcedir
        self.dynamic_length = dynamic_length
        self.task_list = list(task_timepoints.keys())
