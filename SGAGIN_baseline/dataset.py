        ...
        if os.path.isfile(os.path.join(sourcedir, f'{self.filename}.pth')):
            ...
            for task in self.task_list:
                ...
                for subject in tqdm(img_list, ncols=60, desc=f'prep:{task.lower()[:3]}'):
                    timeseries = roi_masker.fit_transform(image.load_img(os.path.join(self.sourcedir, 'img', 'TASK', task, subject))) #image.load_img('./data/img/TASK/RELATIONAL/100206.nii.gz') -> timeseries.shape (232, 116)
                    if not len(timeseries)==task_timepoints[task]:
                        print(f"short timeseries: {len(timeseries)}")
                        continue
                    self.timeseries_list.append(timeseries) # subject가 2명이면 7개의 task니까 len(self.timeseries) == 14,  self.timeseries_list[0].shape == (176, 116)
                    self.label_list.append(task) # subject가 2명이면 7개의 task니까 len(self.label_list) == 14 ['EMOTION', 'EMOTION', 'GAMBLING', 'GAMBLING', 'LANGUAGE', 'LANGUAGE', 'MOTOR', 'MOTOR', 'RELATIONAL', ...]
