...
def step(model, criterion, inputs, label, device='cpu', optimizer=None):
    ...
    # run model
    logit, M, indices = model(inputs.to(device))
...
def train(args):
    ...
    # define dataset
    dataset = DatasetHCPTask(args.sourcedir, roi=args.roi, crop_length=args.crop_length, k_fold=args.k_fold) #전체데이터 7428중 4/5 (train data)에 해당하는 5942
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True) # 5942를 batch8로 나눈 743
    ...
    for k in range(checkpoint['fold'], args.k_fold):
        ...
        model = GMNetwork(args, batch, time, roi, n_labels)
        model.to(device)
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=3e-6, epochs=101, steps_per_epoch=len(dataloader), pct_start=0.2, div_factor=3e-6/1e-6, final_div_factor=1000)
        ...
        for epoch in range(checkpoint['epoch'], args.epochs_ae):
            ...
            for i, x in enumerate(dataloader):
                with torch.no_grad():
                    ...
                    val_loss, val_logit, val_M, val_indices  = step(
                        model=model,
                        criterion=criterion,
                        inputs = inputs,
                        label=label,
                    ...
                    if i == 0:
                        f = open(os.path.join(args.targetdir, 'indices.log'), 'a')
                        writelog(f, 'indices for epoch %d' % epoch)
                        writelog(f, str(val_indices[0]))
                        writelog(f, '----------------------------------------------------------')
                        f.close()
...
if __name__=='__main__':
    ...
    def get_arguments():
        ...
        parser.add_argument('-n', '--exp_name', type=str, default='experiment_2_5_1_test')
        ...
        parser.add_argument("--epochs_ae", type=int, default=3, help="Epochs number of the autoencoder training", )
