...
class DatasetHCPRest(torch.utils.data.Dataset):
    def __init__(self, sourcedir, roi, k_fold=None, target_feature='Gender', smoothing_fwhm=None, regression=False,
                 num_samples=-1):
        super().__init__()
...
class DatasetHCPTask(torch.utils.data.Dataset):
    def __init__(self, sourcedir, roi, crop_length=None, k_fold=None, smoothing_fwhm=None):
        ...
        if os.path.isfile(os.path.join(sourcedir, f'{self.filename}.pth')):
            self.timeseries_list, self.label_list, self.id = torch.load(os.path.join(sourcedir, f'{self.filename}.pth'))
...
class DatasetUKBRest(torch.utils.data.Dataset):
    def __init__(self, sourcedir, roi, k_fold=None, target_feature='31-0.0', smoothing_fwhm=None, regression=False,
                 num_samples=-1):
        super().__init__()
        ...
        if os.path.isfile(os.path.join(sourcedir, f'{self.filename}.pkl')):
            ...
            self.timeseries_dict = {}
            timeseries_list = [f for f in os.listdir(os.path.join(sourcedir, 'roitimeseries', 'ukb_rest')) if
                               f.endswith('.pth')]
    ...
    def __getitem__(self, idx):
        ...
        return {'id': subject, 'timeseries': torch.tensor(timeseries, dtype=torch.float32),
                'label': torch.tensor(label, dtype=torch.float32 if self.num_classes == 1 else torch.int64)}
...
class DatasetABIDE(torch.utils.data.Dataset):
    def __init__(self, sourcedir, roi, k_fold=None, dynamic_length=None, target_feature='DX_GROUP', smoothing_fwhm=None,
                 abide_dir='/mnt/workdisk2/nilearn_data'):
        super().__init__()
        ...
        if roi == 'schaefer':
            ...
            self.roi = fetch_atlas_destrieux_2009(data_dir=os.path.join(sourcedir, 'roi'))
        elif roi == 'harvard_oxford':
            self.roi = fetch_atlas_harvard_oxford(atlas_name='cort-maxprob-thr25-2mm',
                                                  data_dir=os.path.join(sourcedir, 'roi'))
...
class DatasetFMRIPREP(torch.utils.data.Dataset):
    def __init__(self, sourcedir, roi, k_fold=None, task='rest', target_feature='Gender', smoothing_fwhm=None,
                 regression=True, prefix='', num_samples=-1):
        super().__init__()
        ...
        if os.path.isfile(os.path.join(sourcedir, f'{self.filename}.pth')):
            ...
            self.timeseries_dict = {}
            img_list = [f for f in
                        glob(os.path.join(sourcedir, '**', f'*task-{task}*_space-MNI152NLin2009cAsym_*preproc*.nii.gz'),
                             recursive=True)]
            img_list.sort()
            for img in tqdm(img_list, ncols=60):
        ...
        self.label_encoder = LabelEncoder()
        behavioral_df = \
        pd.read_csv(os.path.join(sourcedir, 'participants.tsv'), delimiter='\t').set_index('participant_id')[
            target_feature]
