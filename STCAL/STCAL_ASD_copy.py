...
def Persion_matrix(data):
    ...
    p_list = FC.ravel()[np.flatnonzero(FC)]
    return p_list
...
def extract_feature2(data):
    ...
    for i in range(data.shape[0]):
        data_mean = np.mean(data[i],axis=0)
        outer = np.outer(data_mean,data_mean)
        feat.append(outer)
    return feat
...
def STCAL(max_seq_len, embedding_size):
    ...
    # position embedding
    time_position_embedding = BertEmbeddingsLayer(max_position_embeddings=512, hidden_size=embedding_size, vocab_size=1)(time_input) #TensorShape([None, 90, 190])
    ROI_position_embedding = BertEmbeddingsLayer(max_position_embeddings=512, hidden_size=max_seq_len, vocab_size=1)(ROI_input) #TensorShape([None, 190, 90])
...
if __name__ == '__main__':
    ...
    # ASD_pheno = np.load('Data/ASD/ASD_phenos_norm.npy',allow_pickle=True)
    train_timeseries_list, train_label_list = torch.load('/home/jwlee/HMM/STCAL/data/train_hcp-task_roi-aal.pth')
    test_timeseries_list, test_label_list = torch.load('/home/jwlee/HMM/STCAL/data/test_hcp-task_roi-aal.pth')



    augment_ASD_feats, augment_ASD_labels, augment_ASD_pheno, sk_data, sk_label = data_augment(train_timeseries_list, train_label_list, test_timeseries_list, crops)

    dataset = DatasetHCPTask(args.sourcedir, roi=args.roi, crop_length=args.crop_length, k_fold=args.k_fold, crops=crops)
    ...
    for i_count in range(CV_count):
        K.clear_session()
        kf = StratifiedKFold(n_splits=10, shuffle=True, random_state=2 ** (i_count + 10))
        ...
        for train_index, test_index in kf.split(sk_data, sk_label):
            ...
            ASD_predict = model.predict(x=[x_test, x_test_pheno])
            ASD_pred = [1 if prob >= 0.5 else 0 for prob in ASD_predict]
            ...
            for i in range(len(test_index)):
                ASD_subject = ASD_pred[i * crops:i * crops + crops]
                if np.count_nonzero(ASD_subject) >= crops/2:
                    pred.append(1)
                else:
                    pred.append(0)

                ASD_subject_score = ASD_predict[i * crops:i * crops + crops]
                pred_score.append(np.sum(ASD_subject_score) / crops)
                if np.sum(ASD_subject_score) / crops >= 0.5:
                    pred2.append(1)
                else:
                    pred2.append(0)

                ASD_true_subject = y_test[i * crops:i * crops + crops]
                true_score.append(np.sum(ASD_true_subject) / crops)
                if np.count_nonzero(ASD_true_subject) >= crops/2:
            ...
            ASD_bert.append(metrics.accuracy_score(y_test, ASD_pred))
            ASD_sub_bert.append(metrics.accuracy_score(true_labels, pred))
            ASD_sub_bert2.append(metrics.accuracy_score(true_labels, pred2))
            ASD_auc.append(auc)
            ASD_specificity.append(specificity)
            ASD_sensitive.append(sensivity)

            print("ASD accuracy: " + str(metrics.accuracy_score(y_test, ASD_pred)))
            print("ASD sub accuracy: " + str(metrics.accuracy_score(true_labels, pred)))
            print("ASD sub2 accuracy: " + str(metrics.accuracy_score(true_labels, pred2)))
            print("ASD sub2 auc: " + str(auc))
            print("ASD sub2 specificity: " + str(specificity))
            print("ASD sub2 sensivity: " + str(sensivity))

            print(str(j_count) + " th CV done!")

        ASD_bert_mean.append(np.mean(ASD_bert))
        ASD_sub_bert_mean.append(np.mean(ASD_sub_bert))
        ASD_sub2_bert_mean.append(np.mean(ASD_sub_bert2))
        ASD_auc_mean.append(np.mean(ASD_auc))
        ASD_specificity_mean.append(np.mean(ASD_specificity))
        ASD_sensitive_mean.append(np.mean(ASD_sensitive))


        print("ASD_bert_mean:" + str(ASD_bert_mean))
        print("ASD_sub_bert_mean:" + str(ASD_sub_bert_mean))
        print("ASD_sub2 mean: " + str(ASD_sub2_bert_mean))
        print("ASD_sub2 auc_mean: " + str(ASD_auc_mean))
        print("ASD_sub2 specificity_mean: " + str(ASD_specificity_mean))
        print("ASD_sub2 sensitive_meann: " + str(ASD_sensitive_mean))
