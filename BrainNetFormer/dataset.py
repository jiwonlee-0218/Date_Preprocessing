...
class DatasetHCPTask(Dataset):
    def __init__(self, sourcedir, subtask_type='detail', roi='aal', dynamic_length=None, k_fold=None, smoothing_fwhm=None):
        ...
        print(os.path.join(sourcedir, f'hcp-task_roi-{roi}.pth'))
        if os.path.isfile(os.path.join(sourcedir, f'hcp-task_roi-{roi}.pth')):
            self.timeseries_list, self.label_list = load(os.path.join(sourcedir, f'hcp-task_roi-{roi}.pth'))
        else:
            # roi_masker = NiftiLabelsMasker(load_img(self.roi['maps']))
