...
def step(model, criterion, dyn_t, dyn_a, sampling_points, sampling_endpoints, t, a, label, subtask_label, reg_lambda, reg_subtask, train=True, clip_grad=0.0, device='cpu', optimizer=None, scheduler=None):
    if optimizer is None: model.eval()
    else: model.train()

    #@ main task prediction
    logit, logit_subtask, attention = model(dyn_t.to(device), dyn_a.to(device), t.to(device), a.to(device), sampling_endpoints)
    ...
    if(train):
        if optimizer is not None:
            ...
            if scheduler is not None:
                scheduler.step()
    return logit, logit_subtask, loss, loss_subtask, attention
...
def train(argv):
    ...
    elif argv.dataset=='task': dataset = DatasetHCPTask(argv.sourcedir, subtask_type=argv.subtask_type, roi=argv.roi, dynamic_length=argv.dynamic_length, k_fold=argv.k_fold)
    else: raise
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        for epoch in range(checkpoint['epoch'], argv.num_epochs):
            ...
            for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k} e:{epoch}')): #
                ...
                #@ 向前传播+反向传播
                logit, logit_subtask, loss, loss_subtask, attention = step(
                    model=model,
                    criterion=criterion,
                    dyn_t=dyn_t,
                    dyn_a=dyn_a,
...
def test(argv):
    ...
    for k in range(argv.k_fold):
        ...
        for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k}')):
            with torch.no_grad():
                # process input data
                dyn_t, sampling_points = util.bold.process_dynamic_t(x['timeseries'], argv.window_size, argv.window_stride)
