...
class BrainDecoder(nn.Module):
    def __init__(self, n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums, window_size, dropout=0.5):
        super().__init__()
    ...
    def forward(self, dyn_t, dyn_a, e_s, X_enc, sampling_endpoints):
        ...
        for layer, (G, T, L) in enumerate(zip(self.gnn_layers, self.transformer_modules, self.linear_layers)):
            # graph convolution
            h = G(h, dyn_a_)
            h_bridge = rearrange(h, '(b t n) c -> b t n c', t=num_timepoints, b=minibatch_size, n=num_nodes) # b t n c  torch.Size([2, 34, 116, 128])
...
class BrainNetFormer(nn.Module):
    def __init__(self, n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums, window_size, dropout=0.5):
        ...
        self.decoder = BrainDecoder(n_region, hidden_dim, num_classes, num_heads, num_layers, sparsity, subtask_nums, window_size, dropout)
