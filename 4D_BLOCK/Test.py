...
save_path = '/DataRead/mjy/Dacon/open/JW/model'
#save_path = """Model weight 및 Test prediction 결과 저장할 경로"""
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label_list is not None:
            ...
            image = cv2.imread(f'{path}/test_synthesized/{img_name}') ################################################################################
...
class Model(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.classifier = nn.Sequential(nn.Dropout(0.5), nn.Linear(1000, num_classes))
...
def inference(model_path, device):
    ...
    ## A.Compose([])을 이용하여 이미지와 라벨 각각에 Augmentation을 적용하기 위한 객체를 생성
    test_transform = A.Compose([
        A.Resize(CFG['IMG_SIZE'], CFG['IMG_SIZE']),
        A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), max_pixel_value=255.0, always_apply=False, p=1.0),
        ToTensorV2()
...
#     df['label'] = df.agg(lambda x: f"{x['A']}{x['B']}{x['C']}{x['D']}{x['E']}{x['F']}{x['G']}{x['H']}{x['I']}{x['J']}", axis=1)
#     minor_idx = df[(df['label'] == '1111111011') | (df['label'] == '1110011111') | (df['label'] == '1100000110') | (df['label'] == '0101000011')].index.values
...
def cross_validation_inference(model_path_list):
    ...
    for model_path in model_path_list:
        ...
        probs, preds = inference(model_path, device)  #probabilities, predictions
...
model_path_list = sorted(glob.glob('/DataRead/mjy/Dacon/open/submission/lje_version_main8/*'))
