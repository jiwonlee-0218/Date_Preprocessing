...
class ModuleGARO(nn.Module):
    ...
    def forward(self, x, node_axis=1):
        ...
        x_q = self.embed_query(x.mean(node_axis, keepdims=True)) #torch.Size([34, 2, 1, 128])
        x_k = self.embed_key(x) #torch.Size([34, 2, 116, 128])
        x_graphattention = torch.sigmoid(torch.matmul(x_q, rearrange(x_k, 't b n c -> t b c n'))/np.sqrt(x_q.shape[-1])).squeeze(2)  #torch.Size([34, 2, 1, 116]) -> torch.Size([34, 2, 116])
        return (x * self.dropout(x_graphattention.unsqueeze(-1))).mean(node_axis), x_graphattention.permute(1,0,2)
...
class ModelSTAGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0):
        ...
        self.initial_linear = nn.Linear(input_dim+hidden_dim, hidden_dim) #Linear(in_features=244, out_features=128, bias=True)
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        time_encoding = repeat(time_encoding, 'b t c -> t b n c', n=num_nodes) #torch.Size([34, 2, 128]) -> torch.Size([2, 34, 116, 128])

        h = torch.cat([v, time_encoding], dim=3) #v: torch.Size([2, 34, 116, 116]) h: torch.Size([2, 34, 116, 244])
        h = rearrange(h, 'b t n c -> (b t n) c') #h: torch.Size([7888, 244])
        h = self.initial_linear(h) #h: torch.Size([7888, 244]) -> torch.Size([7888, 128])
        a = self._collate_adjacency(a, self.sparsity) #torch.Size([2, 34, 116, 116]) -> torch.Size([7888, 7888])
        for layer, (G, R, T, L) in enumerate(zip(self.gnn_layers, self.readout_modules, self.transformer_modules, self.linear_layers)):
            h = G(h, a) #torch.Size([7888, 128]) -> torch.Size([7888, 128])
            h_bridge = rearrange(h, '(b t n) c -> t b n c', t=num_timepoints, b=minibatch_size, n=num_nodes) #torch.Size([34, 2, 116, 128]) == paper's H(node feature matrix)
            ...
            h_attend, time_attn = T(h_readout) #time_attn = torch.Size([2, 34, 34])
            ortho_latent = rearrange(h_bridge, 't b n c -> (t b) n c') #torch.Size([68, 116, 128])
            matrix_inner = torch.bmm(ortho_latent, ortho_latent.permute(0,2,1)) #torch.Size([68, 116, 116])
            ...
            attention['node-attention'].append(node_attn) #len(attention['node-attention'])=4 why? layer is 4
            attention['time-attention'].append(time_attn)
            latent_list.append(latent)

        logit = logit.squeeze(1)
        attention['node-attention'] = torch.stack(attention['node-attention'], dim=1).detach().cpu()  #torch.Size([2, 4, 34, 116])
