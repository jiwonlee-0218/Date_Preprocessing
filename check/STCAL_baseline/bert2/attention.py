...
class Attention_translation(Layer):
    ...
    def build(self, input_shape):
        ...
        super(Attention_translation, self).build(input_shape)
    ...
    def compute_output_shape(self, input_shape):
        ...
        # attention_shape = [input_shape[0],self.params.num_heads,input_shape[1],self.params.size_per_head]
        attention_shape = [input_shape[0],self.params.num_heads,input_shape[1],input_shape[1]]
        attention_shape_t = [input_shape[0],self.params.num_heads,input_shape[2],input_shape[2]]


        if self.params.score == True:
            return [value_shape,attention_shape,attention_shape_t]  # [B, F, N*H], [B, F, T]
        else:
            return value_shape
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        def transpose_for_scores_t(input_tensor, seq_len):
            output_shape = [batch_size, seq_len,
                            self.params.num_heads, 19]
            output_tensor = K.reshape(input_tensor, output_shape)
            return tf.transpose(a=output_tensor, perm=[0, 2, 1, 3])  # [B,N,F,H]
        ...
        if self.params.score == True:
            ...
            query_t = transpose_for_scores_t(query, 90)  # [B, N, F, H]
            key_t = transpose_for_scores_t(key, 90)  # [B, N, T, H]
            ...
            attention_scores_t = tf.matmul(query_t, key_t, transpose_b=True)  # [B, N, F, T]
            attention_scores_t = attention_scores_t / tf.sqrt(float(19))

            return [value, attention_scores,attention_scores_t] # [B, F, N*H]
        else:
            value = self.key_layer(to_tensor)  # [B,T, N*H]
            return value
    # noinspection PyUnusedLocal
...
class Attention_SV(Layer):
    ...
    def build(self, input_shape):
        ...
        # B, F, T, N, H - batch, from_seq_len, to_seq_len, num_heads, size_per_head
        #
        self.query_layer = keras.layers.Dense(units=dense_units, activation=self.query_activation, kernel_initializer=self.create_initializer(), name="query")
        self.key_layer = keras.layers.Dense(units=dense_units, activation=self.key_activation, kernel_initializer=self.create_initializer(), name="key")
        self.value_layer = keras.layers.Dense(units=dense_units, activation=self.value_activation, kernel_initializer=self.create_initializer(), name="value")
        self.dropout_layer = keras.layers.Dropout(self.params.attention_dropout)
...
class Attention_QKV(Layer):
    ...
    def build(self, input_shape):
        ...
        super(Attention_QKV, self).build(input_shape)
    ...
    def compute_output_shape(self, input_shape):
        ...
        value_shape = input_shape
        if self.params.value_key == True:
            return [query_shape,key_shape,value_shape]  # [B, F, N*H], [B, F, T]
        else:
            return query_shape
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        if self.params.value_key == True:
            ...
            key = self.key_layer(to_tensor)  # [B,T, N*H]
            value = self.key_layer(to_tensor)  # [B,T, N*H]
            return [query, key, value]  # [B, F, N*H]
        else:
            query = self.query_layer(from_tensor)  # [B,F, N*H] [batch_size, from_seq_len, N*H]
            return query # [B, F, N*H]
