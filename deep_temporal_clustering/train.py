...
def train_ClusterNET(args, epoch):
    """
    ...
    for batch_idx, (inputs, labels) in enumerate(train_dl):  # epoch 1에 모든 training_data batch만큼
        ...
        #     loss = loss_function(fp[i], fq[i])
        #     sum_kl_loss += loss
        # loss_KL = sum_kl_loss / fp.size(0)
...
def training_function(args, verbose=True):
    ...
    for epoch in range(args.max_epochs):
        train_loss, mse_loss, kl_loss = train_ClusterNET(args, epoch) # 1 epoch training
