...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.attn_drop(attn)   # attn: torch.Size([8, 3, 285, 285])

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn    # x: torch.Size([8, 285, 90]), attn: torch.Size([8, 3, 285, 285])
...
class Block(nn.Module):
    ...
    def forward(self, x):   #-------------------------modified for attention map
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=1, in_chans=116, embed_dim=90):
        super().__init__()
...
class VisionTransformer(nn.Module):   # img_size [284]
    ...
    def __init__(self, img_size=[224], patch_size=1, in_chans=116, num_classes=0, embed_dim=90, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))  # torch.Size([1, 286, 116])
    ...
    def forward(self, x, classify=False):
        attntion_maps = []   #-------------------------modified for attention map
        ...
        for i, blk in enumerate(self.blocks):  # block 0~11  transformer encoder
            ...
            x, attn = blk(x)   #-------------------------modified for attention map
            attntion_maps.append(attn)   #-------------------------modified for attention map

        x = self.norm(x)       # torch.Size([64, 286, 80])  partial(nn.LayerNorm, eps=1e-6)
        return x, attntion_maps #-------------------------modified for attention map
...
class RECHead(nn.Module):
    ...
    def forward(self, x):
        ...
        x_rec = x.transpose(1, 2)  # torch.Size([8, 90, 284])
        x_rec = self.convTrans(x_rec)  # torch.Size([8, 116, 284])
        x_rec = x_rec.permute(0, 2, 1)  # torch.Size([8, 284, 116])
