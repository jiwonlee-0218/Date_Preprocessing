...
from test_fMRI_vision_transformer_SiT import RECHead
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument("--dataset_name", default="MOTOR_32", help="dataset name")
    ...
    # Attention
    parser.add_argument("--return_attention", type=bool, default=True, help="for attention map")

    # directory
    parser.add_argument("--dir_root", default='/home/jwlee/HMM/deep_temporal_clustering/SiT/', )
    parser.add_argument("--model_name", default="SiT_for_test", help="model name")
...
def test_function(args, X_test_data):
    ...
    with torch.no_grad():
        ...
        test_recons_l, test_recons_l_w, test_rec_imgs, test_orig_imgs, test_represent = test_SiT_model(X_test_data)
...
class FullpiplineSiT(nn.Module):
    ...
    def __init__(self, args, backbone):
        ...
        self.return_attn = args.return_attention
    ...
    def forward(self, im):
        ...
        # attention_maps each element is a tensor of shape [Batch, Heads, SeqLen, SeqLen]
        encoded_out, attention_maps = self.backbone(im) # torch.Size([8, 284, 116]) -> torch.Size([8, 286, 80])  attention_maps={list:12} -> attention_maps[0]: torch.Size([Batch, 3, 285, 285])
        represent = encoded_out[:, 1:]
        ...
        if self.rec == 1:
            recons_imgs = self.rec_head(encoded_out[:, 1:])  # torch.Size([8, 284, 90])가 rec_head로 들어간다 -> torch.Size([8, 284, 116])
            recloss = F.l1_loss(recons_imgs, im, reduction='none') ############# loss
        ...
        if self.return_attn:
            if not os.path.exists(os.path.join(directory, 'SiT_models_attn_pictures/')):
                os.makedirs(os.path.join(directory, 'SiT_models_attn_pictures/'))

            attn_maps = [ m[1].detach().cpu().numpy() for m in attention_maps ] # m은 attention layer 하나하나->torch.Size([108, 3, 285, 285]) , m[0]은 0번 subject -> (3, 285, 285)
            # attn_maps={list:12} -> attn_maps[0]: (3, 285, 285)
            ...
            for row in range(num_layers):
                for column in range(num_heads):
                    plt.title(f"Layer {row+1}, Head {column+1}")
                    plt.imshow(attn_maps[row][column])
                    plt.colorbar()
                    plt.savefig(os.path.join(directory, 'SiT_models_attn_pictures/') + 'layer %s head %s.png' % (str(row),str(column)))
                    plt.close()
        ...
        return loss_rec, loss_rec_w, recons_imgs, im, represent
