...
from test_pytorchtools import EarlyStopping
...
def train_SiTv2(args, train_dl, valid_dl, directory):
    ...
    for epoch in range(args.epochs):
        ...
        for it, (im, _) in enumerate(train_dl):
            ...
            recons_l, recons_l_w, rec_imgs, orig_imgs, represent = SiT_model(im)
        ...
        with torch.no_grad():
            ...
            for j, (val_x, val_y) in enumerate(valid_dl):
                ...
                val_recons_l, val_recons_l_w, val_rec_imgs, val_orig_imgs, val_represent = SiT_model(val_x)
...
def training_function(args, directory, train_dl, valid_dl, verbose=True):
    ...
    DTC_model = test_clusternet.ClusterNet(args, path=path)  ## 모델 초기화 with the pretrained autoencoder model 정의해놓고 입력넣으면 바로 쓸 수 있게
