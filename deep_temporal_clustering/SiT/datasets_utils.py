    ...
    for si, s in enumerate(samples):
        ...
        for i in range(n_imgs):
            ...
            if random.random() < rep_drop:
                samples_aug[i], masks[i] = GMML_drop_rand_patches(samples_aug[i], samples[si][idx_rnd],
                                                                  max_replace=max_replace, align=align)
            else:
...
class DataAugmentationSiT(object):
    ...
    def __call__(self, image):
        ...
        # corrupted
        if self.drop_perc > 0:
            im_corr1, im_mask1 = GMML_drop_rand_patches(im1.detach().clone(), max_replace=self.drop_perc,
                                                        drop_type=self.drop_type, align=self.drop_align)
        else:
        ...
        # corrupted
        if self.drop_perc > 0:
            im_corr2, im_mask2 = GMML_drop_rand_patches(im2.detach().clone(), max_replace=self.drop_perc,
                                                        drop_type=self.drop_type, align=self.drop_align)
        else:
