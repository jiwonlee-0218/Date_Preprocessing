...
class TAE(nn.Module):
    ...
    def __init__(self, args, filter_1=50, filter_lstm=[50, 10]):  # 내 모델에 사용될 구성품을 정의 및 초기화하는 메서드
...
class TAE_encoder(nn.Module):
    ...
    def forward(self, x):   # x : (1, 284, 116)
        ...
        out_lstm1 = torch.sum(out_lstm1.view(out_lstm1.shape[0], out_lstm1.shape[1], 2, self.hidden_lstm_1),dim=2, )  # (1, 284, 50)

        out_lstm2, _ = self.lstm_2(out_lstm1)
        out_lstm2 = torch.sum(out_lstm2.view(out_lstm2.shape[0], out_lstm2.shape[1], 2, self.hidden_lstm_2),dim=2, )  # (1, 284, 10) ## (batch_size , n_hidden ,10)
...
#             bidirectional=True
#         )
#         self.act_lstm_2 = nn.LeakyReLU()
#
#     def forward(self, x):  # x : (Batch, 284, 116)
#
#         ## encoder
#         x = x.transpose(1,2)   # (Batch, 116, 284)
#         out_cnn = self.conv_layer(x)   # (Batch, 64, 284)
#         out_cnn = out_cnn.permute(0, 2, 1)   # (Batch, 284, 64)
#
#         out_lstm1, _ = self.lstm_1(out_cnn) # (Batch, 284, 100)
#         out_lstm1 = self.act_lstm_1(torch.sum(out_lstm1.view(out_lstm1.shape[0], out_lstm1.shape[1], 2, self.hidden_lstm_1),dim=2,)) # (1, 284, 50)
#
#
#         features, _ = self.lstm_2(out_lstm1) # (1, 284, 20)
#         features = self.act_lstm_2(torch.sum(features.view(features.shape[0], features.shape[1], 2, self.hidden_lstm_2),dim=2,))  # (1, 284, 10) ## (batch_size , n_hidden ,10)
#
#
#         return features
#
...
#     """
#
#     def __init__(self, args, filter_1=64, filter_lstm=[50, 10]):  # 내 모델에 사용될 구성품을 정의 및 초기화하는 메서드
#         super(TAE, self).__init__()
#
#
#         self.filter_1 = filter_1
...
class ClusterNet(nn.Module):
    ...
    def __init__(self, args,  cluster_centers = None, path='.', filter_1=64, filter_lstm=[50, 10]):
        ...
        # self.centroids = nn.Parameter(torch.zeros(8, 32), requires_grad=True) ## optimizer의 model.parameters()에 들어가도록
    ...
    def forward(self, x, all_gt):
        ...
        z, x_reconstr = self.tae(x) # z: (8, 284, 32), x_reconstr: (8, 284, 116)
        z_np = z.detach().cpu() # z_np: (8, 284, 32)
        ...
        for i in range(x.size(0)):  # x.size(0): 8 == batch_size, x: (8, 284, 116)
            # X_tensor = z[i].type(torch.FloatTensor).to(self.device)  # z_np[i] : torch.Size([284, 32]) represented



            similarity = compute_similarity(
                z[i], self.cluster_centers, similarity=self.similarity   # centroid와 representation간의 거리 계산
            ) ## similarity torch.Size([284, 8])
            ...
            # loss_KL = self.loss_function(final_Q, final_P)
