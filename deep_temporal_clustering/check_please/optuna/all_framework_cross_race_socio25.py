...
def trnvaltst(data, seed, fold, race, socio):
    # Load index
    idxg_path = main_path + 'race_threshold25_cv/rp%s_r%d_god_%s_%s.npz' % (seed, race, socio[0], socio[1])
    idxb_path = main_path + 'race_threshold25_cv/rp%s_r%d_bad_%s_%s.npz' % (seed, race, socio[0], socio[1])
...
if __name__ == '__main__':
    ...
    print(result)
    with open("/home/eskang/3T/Socio_Brain_2023/LinearSVC_thr25/race%d_%s_%s.txt" % (args.race, socio_set[0], socio_set[1]), 'a') as f:
        for r in result:
            f.write(str(r) + ' ')
        f.write('\n')
        f.close()
