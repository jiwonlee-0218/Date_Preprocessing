...
def trnvaltst(data, seed, fold, race, socio):
    ...
    idxg_path = main_path + 'race_threshold10_cv/rp%s_r%d_god_%s.npz' % (seed, race, socio)
    idxb_path = main_path + 'race_threshold10_cv/rp%s_r%d_bad_%s.npz' % (seed, race, socio)

    idxg_1 = np.load(idxg_path, allow_pickle=True)
    idxb_1 = np.load(idxb_path, allow_pickle=True)

    trnidx_g, trnidx_b = idxg_1['trn_idx'][fold], idxb_1['trn_idx'][fold]   # trn_idx, val_idx, tst_idx
...
class Objective:
    ...
    def __call__(self, trial):
        ...
        X_train, X_valid, X_test, y_train, y_valid, y_test = trnvaltst(conn_data, args.seed, args.fold, args.race, args.socio)
        X_train, X_valid, X_test = normalization2(X_train, X_valid, X_test, args.norm2)
...
def test(data, args, best_param):
    X_train, X_valid, X_test, y_train, _, y_test = trnvaltst(data, args.seed, args.fold, args.race, args.socio)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--case", type=str, default='q', choices=['s', 'd', 'q'])
    parser.add_argument("--race", type=int, default=2, choices=[1, 2, 3], help='1: white, 2: black, 3: hispanic')
    ...
    parser.add_argument("--socio", type=str, default='i_educ', choices=['i_educ', 'i_income', 'n_edu_h', 'n_pov'])
    ...
    # conn_path = main_path + 'con_aparc_count_nanx_threshold500_clustering_coef.csv'
    # conn_path = main_path + 'con_aparc_count_nanx_threshold100_degree_centrality.csv'
    conn = pd.read_csv(conn_path).to_numpy()[:, 2:] #(9099, 3486)
    # conn[conn < 500] = 0
    conn = normalization(conn, args.norm) #(9099, 3486)
    ...
    study.optimize(Objective(args=args, data=conn), n_trials=50)

    result = test(conn, args, study.best_params)
    print(result)
    with open("/home/eskang/3T/Socio_Brain_2023/LinearSVC/race%d_%s.txt" % (args.race, args.socio), 'a') as f:
        for r in result:
            f.write(str(r) + ' ')
        f.write('\n')
        f.close()
