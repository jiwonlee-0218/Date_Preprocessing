...
import os
import numpy as np
from cluster_2_motor_main_config import get_arguments
from cluster_2_motor_models import ClusterNet, TAE
...
def train_ClusterNET(epoch, args, verbose):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_dl):  # epoch 1에 모든 training_data batch만큼
        ...
        z, x_reconstr, fp, fq, ARI = model(inputs, all_gt)  # ClusterNet의 forward
        ...
        total_loss = loss_mse + ( 1000 * loss_KL)
        total_loss.backward()
    ...
    for epoch in range(args.max_epochs):
        ...
        with torch.no_grad():
            ...
            for j, (val_x, val_y) in enumerate(valid_dl):
                ...
                z, x_reconstr, fp, fq, ARI = model(val_x, all_gt)

                V_loss_mse = loss_clu(val_x, x_reconstr)
                V_loss_KL = kl_loss_function(fp, fq)
                # V_loss_KL = loss_function(fq, fp)
                V_total_loss = V_loss_mse + ( 1000 * V_loss_KL)
...
####################################################
    ...
    for i in list_atlas:
        b = sample[:,:, i:i+2]
        arr = np.concatenate((arr, b), axis=2)

    motor_sample = arr   # (1080, 284, 10)




    LR_list = [36, 78, 99, 162, 183, 225, 246]
    arr = motor_sample[:, 15:15 + 17, :]

    for i in LR_list:
        b = motor_sample[:,i:i+17,:]
        arr = np.concatenate((arr, b), axis=1)

    cluster_2_motor_sample = arr   # (1080, 136, 10)
#####################################################

    for i in range(1080):

        label_list = [0 for i in range(136)]
        right_index = [0, 34, 85, 119]
        left_index = [17, 51, 68, 102]

        for id in right_index:
            label_list[id: id + 17] = [1 for i in range(17)]

        for id in left_index:
            label_list[id: id + 17] = [2 for i in range(17)]
    ...
    cluster_2_motor_label = label_list_RL  # (1080, 136)
    #####################################################
    ...
    X_train, X_test, y_train, y_test = train_test_split(cluster_2_motor_sample, cluster_2_motor_label, random_state=42, test_size=0.2)
    ...
    if args.ae_weights is not None and args.ae_models is not None:  ### weight ok, model ok
        ...
        with torch.no_grad():
            ...
            z, x_reconstr, fp, fq, ARI = dtc_model(X_test, all_gt)
