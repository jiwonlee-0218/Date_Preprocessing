...
import os
import numpy as np
from main_config2 import get_arguments
from models2 import ClusterNet, TAE
...
def train_ClusterNET(epoch, args, verbose):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_dl):  # epoch 1에 모든 training_data batch만큼
        ...
        z, x_reconstr, fp, fq, ARI = model(epoch, inputs, all_gt)  # ClusterNet의 forward
    ...
    for epoch in range(args.max_epochs):
        ...
        with torch.no_grad():
            ...
            for j, (val_x, val_y) in enumerate(valid_dl):
                ...
                z, x_reconstr, fp, fq, ARI = model(epoch, val_x, all_gt)
...
#####################################################
    ...
    X_train, X_test, y_train, y_test = train_test_split(motor_sample, label, random_state=42, test_size=0.2)
    ...
    if args.ae_weights is not None and args.ae_models is None and args.autoencoder_test is None:  #### weight ok, but model is none
        ...
        # path = os.path.join(directory, args.ae_weights)
        path = '/home/jwlee/HMM/deep_temporal_clustering/Only_Motor_Area/new_Autoencoder_onedir_LSTM32/AAL_116/Epochs300_BS_8_LR_0.0001_wdcay_1e-06/models_weights/checkpoint_epoch_300_loss_0.00428.pt'
    ...
    if args.ae_weights is not None and args.ae_models is not None:  ### weight ok, model ok
        ...
        with torch.no_grad():
            ...
            z, x_reconstr, fp, fq, ARI = dtc_model(epoch, X_test, all_gt)
