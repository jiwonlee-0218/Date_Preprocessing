...
class VisionTransformer(nn.Module):   # img_size [284]
    ...
    def __init__(self, img_size=224, patch_size=1, in_chans=116, embed_dim=90, depth=12,
        ...
        self.mask_token = torch.zeros((1, 1, 116), device=device)
    ...
    def prepare_tokens(self, x):
        ...
        x_ = x_ + self.interpolate_pos_encoding(x_, w, h)   # add positional encoding to each token (16, 284, 64) + (1, 284, 64)

        return self.pos_drop(x_)

    def forward(self, x):    # encoding layer

        x_ = self.masking_tokens(x, 0.7)
