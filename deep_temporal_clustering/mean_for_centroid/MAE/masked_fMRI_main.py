...
from masked_fMRI_train import *
import masked_fMRI_clusternet
...
def get_args_parser():
    ...
    parser.add_argument("--model_name", default="149first_masked_test_70", help="model name")
    ...
    parser.add_argument("--max_epochs", type=int, default=400, help="Maximum epochs numer of the full model training", )
    ...
    # directory
    parser.add_argument("--dir_root", default='/home/jwlee/HMM/deep_temporal_clustering/mean_for_centroid/MAE/', )
...
if __name__ == '__main__':
    ...
    directory = os.path.join(args.dir_root, args.model_name, args.dataset_name,
                             'Epochs' + str(args.max_epochs) + '_BS_' + str(args.batch_size) + '_LR_' + str(
    ...
    if args.weights is not None and args.SiT_models is None and args.SiT_test is None:  #### weight ok, but model is none
        ...
        # full_path = '/home/jwlee/HMM/deep_temporal_clustering/mean_for_centroid/MAE/new_masked_test_70/MOTOR/Epochs150_BS_16_LR_0.001_wdcay_1e-06/models_weights/checkpoint_epoch_149_loss_0.01362.pt'
        # print("I got: " + full_path + " weights")
        ...
        DTC_model = masked_fMRI_clusternet.ClusterNet(args, Full_SiT_model)
        DTC_model = DTC_model.to(args.device)
