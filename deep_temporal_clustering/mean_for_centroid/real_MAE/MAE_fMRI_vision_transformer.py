...
class PositionalEncoding(nn.Module):
    def __init__(self, emb_size: int, maxlen: int = 284):
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, H, W = x.shape # (16, 284, 116)
        x = x.transpose(1,2)  # (16, 116, 284)
        x = self.proj(x).transpose(1, 2)  # (16, 64, 284) -> (16, 284, 64)
        return x  # (16, 284, 64) patch sequence = 284 patch 1개당 80 dimension
...
class VisionTransformer(nn.Module):   # img_size [284]
    ...
    def __init__(self, img_size=224, patch_size=1, in_chans=116, embed_dim=90, depth=12,
        ...
        self.mask_token = torch.zeros((1, 1, 116), device=self.device)




    def generative_masking(self, x_s, mask_ratio):
        x, mask, ids_restore = self.random_masking(x_s, mask_ratio)
    ...
    def forward(self, x):    # encoding layer
        ...
        x_ = self.generative_masking(x, 0.3)


        x_ = self.prepare_tokens(x_)   # torch.Size([16, 286, 64])  embed 1단계
