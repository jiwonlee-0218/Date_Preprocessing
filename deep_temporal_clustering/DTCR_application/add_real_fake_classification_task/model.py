...
class TAE_encoder(nn.Module):
    ...
    def forward(self, x):  # x : (1, 284, 116)
        outputs = []
        ## encoder
        x = x.transpose(1,2) # (1, 116, 150)
        out_cnn = self.conv_layer(x) # (1, 64, 150)
        out_cnn = out_cnn.permute(0, 2, 1) # (1, 150, 64)

        out_lstm1, h_n1 = self.lstm_1(out_cnn) # (1,284, 100)
        out_lstm1_act = self.act_lstm_1(out_lstm1)
        outputs.append(h_n1[0])

        features, h_n2 = self.lstm_2(out_lstm1_act) # (1, 284, 20)
        out_lstm2_act = self.act_lstm_2(features)
        outputs.append(h_n2[0])

        encoder_state_fw = torch.concat(outputs, dim=2)

        return out_lstm2_act, encoder_state_fw
...
class Classifier(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #inputs: torch.Size([1, 16, 82]) -> torch.Size([16, 82])
...
class TAE_decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        ## decoder
        features = features.transpose(1, 2)  #torch.Size([16, 32, 150])
        out_deconv = self.deconv_layer(features)  #torch.Size([16, 116, 150])
        out_deconv = out_deconv.permute(0,2,1)  #torch.Size([16, 150, 116])
        return out_deconv
...
class TAE(nn.Module):
    ...
    def __init__(self, args, filter_1=64, filter_lstm=[50, 32], in_features=82, hidden_units=[128, 2]):  # 내 모델에 사용될 구성품을 정의 및 초기화하는 메서드
        ...
        self.tae_classifier = Classifier(
    ...
    def forward(self, x):        #  init에서 정의된 구성품들을 연결하는 메서드
        ...
        features, encoder_state_fw = self.tae_encoder(x)
        classifier_output = self.tae_classifier(encoder_state_fw) #torch.Size([16, 2]) real + fake
        out_deconv = self.tae_decoder(features)
        return features, out_deconv, classifier_output   # features는 clustering을 위해 encoder의 output을 사용
...
class ClusterNet(nn.Module):
    ...
    def forward(self, x, all_gt):
        ...
        kl_loss = (all_kl_loss / z_np.size(0))
        all_batch_ARI = (ARI / (x.size(0)))  # batch_size에 대한 ARI

        return z, x_reconstr, kl_loss, all_batch_ARI  # kl loss, all_preds = list이므로 batch_idx=0일 때, len(list)=1, list[0].shape = torch.Size([64, 284])
