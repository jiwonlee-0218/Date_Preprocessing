...
def cluster_using_gmm(embeddings, K):
    ...
    kmc = GaussianMixture(n_components=K, covariance_type='diag').fit(embeddings)
    pmc = kmc.predict(embeddings)
    return pmc


def cluster_using_spectral(embeddings, K):

    kmc = SpectralClustering(n_clusters=K).fit(embeddings)
    pmc = kmc.predict(embeddings)
    return pmc