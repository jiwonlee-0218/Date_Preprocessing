...
class Classifier(nn.Module):
    ...
    def __init__(self, in_features, hidden_units):
        ...
        self.fc1 = nn.Linear(in_features=3000, out_features=self.hidden_units_1, bias=False)
        self.fc2 = nn.Linear(in_features=self.hidden_units_1, out_features=self.hidden_units_2, bias=False)


    def forward(self, inputs):

        x = inputs.flatten(1, 2)
        #inputs: torch.Size([16, 400])
        out_cnn = self.fc1(x)
        out = self.fc2(out_cnn)

        return out

class Encoder(nn.Module):
    def __init__(self, n_input, filter_1, n_hidden):
        ...
        self.drnn = DRNN(n_input=filter_1, n_hidden=n_hidden, n_layers=3, cell_type='GRU')
    ...
    def forward(self, inputs):
        ...
        outputs_fw, states_fw = self.drnn(out_cnn)
...
class Seq2Seq(nn.Module):
    def __init__(self, args, n_input, filter_1, n_hidden, classification_unit):
        ...
        # self.f_classifier = Classifier(in_features=np.sum(n_hidden), hidden_units=classification_unit)
        self.f_classifier = Classifier(in_features=150, hidden_units=classification_unit) ############
    ...
    def forward(self, inputs, _):
        ...
        true_enc_output = f_outputs[_[:,0]==1]
        h_real = f_final_hidden_state[_[:,0]==1]

        aa = true_enc_output.reshape(-1, 20) ##################
        kmeans_loss = self.kmeans(aa) ##################
        # kmeans_loss = self.kmeans(h_real)
        # classifier_output = self.f_classifier(f_final_hidden_state)
        classifier_output = self.f_classifier(f_outputs) #################
        true_input_recons = self.f_dec(true_enc_output)
        # input_recons = self.f_dec(f_outputs)


        # return h_real, true_input_recons, classifier_output, kmeans_loss
        return aa, true_input_recons, classifier_output, kmeans_loss #####################
