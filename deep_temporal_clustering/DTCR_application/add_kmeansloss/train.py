...
def pretrain_autoencoder(args, train_dl, test_dl, kmeans_train, kmeans_test, directory='.'):
    ...
    for epoch in tqdm(range(args.epochs_ae)):
        ...
        if epoch % 5 == 0 and epoch != 0:
            ...
            topk_evecs = Vh[sorted_indices.flip(dims=(0,))[:6], :]
            F_new = topk_evecs.T
        ...
        print()
        print("total loss for epoch {} is : {}, recon loss: {}, kmeans loss: {}".format(epoch + 1, train_loss, recon_loss, km_loss))
        torch.save({
        ...
        with torch.no_grad():
            ...
            for t_idx, (train, train_label) in enumerate(kmeans_train):
                ...
                if epoch == 0 or (epoch+1) == 50 or (epoch+1) == 100 or (epoch+1) == 150 or (epoch+1) == 200 or (epoch+1) == 250 or (epoch+1) == 300 or (epoch+1) == 400 or (epoch+1) == 450 or (epoch+1) == 500:
                    if (t_idx+1) == 100:
...
if __name__ == "__main__":
    ...
    if args.ae_weights is not None and args.epochs_ae > 0:
        ...
        pred_train = utils.cluster_using_kmeans(features, args.n_clusters)
        RI = utils.ri_score(sub100_label, pred_train)
        NMI = utils.nmi_score(features, pred_train)
        tsne = TSNE(n_components=2)
