...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument("--dataset_name", default="EMOTION", help="dataset name")
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = train_test_split(sample, labels, random_state=42, test_size=0.2)
    ...
    emotion_sub, emotion_labels = next(iter(train_dl))
    print('SUB: ', emotion_sub.shape, 'LABEL: ', emotion_labels.shape)

    print(f"-------> The dataset consists of {len(train_dl) * args.batch_size} subjects.")  ## 864 subject
    print(f"-------> The dataset size: {emotion_sub[0, :, :].shape}")
    ...
    if args.weights is not None and args.SiT_models is None and args.SiT_test is not None:
        ...
        writelog(f, '----------------------')
        writelog(f, 'Data Name: %s' % args.dataset_name)
        writelog(f, '----------------------')
