...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    """
    def __init__(self, n_input, n_hidden, compared_length, nb_random_samples, negative_penalty,
                 batch_size, nb_steps, lr, penalty, early_stopping,
                 encoder, params, in_channels, out_channels, cuda=True,
                 gpu=0, epochs=3):
        self.architecture = ''
        self.cuda = cuda
        self.gpu = gpu
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        train_generator = torch.utils.data.DataLoader(
            train_torch_dataset, batch_size=self.batch_size
        )
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            for batch_idx, batch in enumerate(train_generator):
                ...
                if not varying:
                    loss = self.loss(
                        batch, self.encoder, save_memory=save_memory
                    )
                else:
            ...
            contrastive_loss = contrastive_loss / (batch_idx+1)
            print('Epoch: {}, contrastive loss: {}'.format(epoch, contrastive_loss))
            ...
            if self.early_stopping is not None and y is not None and (ratio >= 5 and train_size >= 50):
                ...
                features = self.encode(X)  #여기서도 사용되기 때문에 eval()로 전환했다가 다시 train()으로 전환해야 된다.
    ...
    def fit(self, X, y, save_memory=False, verbose=False):
        ...
        @param verbose Enables, if True, to monitor which epoch is running in
               the encoder training.
        """
        # Fitting encoder, training된 encoder
    ...
    def encode(self, X, batch_size=50):
        ...
        features = numpy.zeros((numpy.shape(X)[0], self.out_channels))  #batch x dimension
        self.encoder = self.encoder.eval() #encoder 평가
        ...
        self.encoder = self.encoder.train() #encoder 학습
        return features  #batch x dimension
...
class CausalCNNEncoderClassifier(TimeSeriesEncoderClassifier):
    ...
    @param gpu GPU index to use, if CUDA is enabled.
    """
    def __init__(self, args, n_input=100, n_hidden=[100, 50, 50], compared_length=50, nb_random_samples=10,
                 negative_penalty=1, batch_size=1, nb_steps=600, lr=0.001,
                 penalty=1, early_stopping=None, channels=[96, 64, 32], depth=1,
                 reduced_size=32, out_channels=10, kernel_size=4,
                 in_channels=96, cuda=True, gpu=0, epochs=100):

        super(CausalCNNEncoderClassifier, self).__init__(n_input, n_hidden,
            compared_length, nb_random_samples, negative_penalty, batch_size,
            nb_steps, lr, penalty, early_stopping,
            self.__create_encoder(args, n_input, n_hidden, in_channels, channels, depth, reduced_size,
                                  out_channels, kernel_size, cuda, gpu),
            self.__encoder_params(in_channels, channels, depth, reduced_size,
                                  out_channels, kernel_size),
            in_channels, out_channels, cuda, gpu, epochs
        )
    ...
    def __create_encoder(self, args, n_input, n_hidden, in_channels, channels, depth, reduced_size,
                         out_channels, kernel_size, cuda, gpu):
        encoder = causal_cnn.ContrastiveTCN(args, in_channels, channels, kernel_size)


        # encoder = causal_cnn.drnn(n_input, n_hidden, in_channels, channels, depth, reduced_size, out_channels,
        #     kernel_size)
        encoder.float() ############################## univariate인 경우 encoder.double(), multivariate인 경우 encoder.float()
        if cuda:
            encoder.cuda(gpu)
        return encoder
