...
def tt_function(args):
    ...
    for k in range(args.k_fold):
        ...
        predict_all = np.array([], dtype=int)
        labels_all = np.array([], dtype=int)
        fold_fc_attention = {'temporal-result': [], 'spatial-result': []}
        ...
        for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k}')):
            with torch.no_grad():
                ...
                fold_fc_attention['spatial-result'].append(np.stack(test_attention['spatial-result'], axis=0))
...
def tt_analysis(args):
    ...
    spatial_result, temporal_result = {}, {}
    ...
    for task in task_list:
        sr = (spatial_result[task].mean(0) - np.min(spatial_result[task].mean(0))) / (np.max(spatial_result[task].mean(0)) - np.min(spatial_result[task].mean(0)))
        plt.imshow(sr)
        plt.colorbar()
        plt.savefig(os.path.join(analysisdir, 'spatial-result', f'spatial_result_{task}.png'))
        plt.close()
...
if __name__=='__main__':
    ...
    def get_arguments():
        ...
        parser.add_argument('-n', '--exp_name', type=str, default='experiment_renewal2_1')
        ...
        parser.add_argument('-dt', '--targetdir', type=str, default='/DataCommon2/jwlee/2023_08_03_ST_Adaptive_Number_Of_Clusters/attention_FC_10/result')
