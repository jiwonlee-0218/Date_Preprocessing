...
## 1
class GMPool_extract_FC(nn.Module):
    def __init__(self, args, batch, time, roi):
        super(GMPool_extract_FC, self).__init__()
    ...
    def forward(self, X):
        ...
        clf_output = self.mlp(distance.view(-1, self.roi)).view(-1, self.time, self.time)
        ...
        for i in range(grouping_M.size(0)):
            ...
            # group_indices[mask] = 1
            # group_indices[~mask] = 0
            # new_X = X[i] * group_indices
            ...
            corr1 = torch.corrcoef(new_X)
            subejct_fc_vectors.append(corr1)

        FC_output = torch.stack(subejct_fc_vectors) #torch.Size([8, 116, 116])
        flattened_FC_output = flatten_upper_triang_values(FC_output) #torch.Size([8, 6670])

        return flattened_FC_output, grouping_M, soft_indices
...
## 2
class GMPool_grouping(nn.Module):
    def __init__(self, args, batch, time, roi):
        super(GMPool_grouping, self).__init__()
    ...
    def forward(self, X):
        ...
        flattened_FC_output = flatten_upper_triang_values(grouping_M) #torch.Size([8, 176, 176]) -> torch.Size([8, 15400])

        return flattened_FC_output, grouping_M
...
class GMPool_masking_RT(nn.Module):
    ...
    def forward(self, X):
        ...
        for i in range(grouping_M.size(0)):
            ...
            subejct_fc_vectors.append(new_X)
...
class Transformer2(nn.Module):
    ...
    def forward(self, text: torch.Tensor) -> torch.Tensor:
        ...
        """
        # get padding mask
        # mask = get_padding_mask(text) input length 길이가 다를때
