...
def training_function(args):
    ...
    for k in range(checkpoint['fold'], args.k_fold):
        ...
        hidden_dim = 128
        model = GMNetwork(roi, hidden_dim, time, n_labels)
        model.to(device)
...
def tt_function(args):
    ...
    for k in range(args.k_fold):
        ...
        predict_all = np.array([], dtype=int)
        labels_all = np.array([], dtype=int)
        fold_fc_attention = {'temporal-result': [], 'spatial-result': [], 'time-attention': [], 'spatial-attention': []}
        ...
        for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k}')):
            with torch.no_grad():
                ...
                fold_fc_attention['temporal-result'].append(test_attention['temporal-result'].detach().cpu().numpy())
                fold_fc_attention['spatial-result'].append(test_attention['spatial-result'].detach().cpu().numpy())
                fold_fc_attention['time-attention'].append(test_attention['time-attention'].detach().cpu().numpy())
                fold_fc_attention['spatial-attention'].append(test_attention['spatial-attention'].detach().cpu().numpy())
...
def tt_analysis(args):
    ...
    os.makedirs(os.path.join(analysisdir, 'time-attention'), exist_ok=True)
    os.makedirs(os.path.join(analysisdir, 'spatial-attention'), exist_ok=True)

    temporal_result, spatial_result, time_attention, spatial_attention = {}, {}, {}, {}
    ...
    for task in task_list:
        ...
        spatial_result[task] = np.concatenate([np.load(os.path.join(args.targetdir, 'attention', str(k), 'spatial-result', f'{task}.npy')) for k in range(args.k_fold)])
        time_attention[task] = np.concatenate([np.load(os.path.join(args.targetdir, 'attention', str(k), 'time-attention', f'{task}.npy')) for k in range(args.k_fold)])
        spatial_attention[task] = np.concatenate([np.load(os.path.join(args.targetdir, 'attention', str(k), 'spatial-attention', f'{task}.npy')) for k in range(args.k_fold)])



    for task in task_list:
        plt.imshow(temporal_result[task].mean(0))
        plt.savefig(os.path.join(analysisdir, 'temporal-result', f'task_temporal_result_{task}.png'))
        plt.close()

        plt.imshow(spatial_result[task].mean(0))
        plt.savefig(os.path.join(analysisdir, 'spatial-result', f'task_spatial_result_{task}.png'))
        plt.close()

        plt.imshow(time_attention[task].mean(0))
        plt.savefig(os.path.join(analysisdir, 'time-attention', f'task_time_attention_{task}.png'))
        plt.close()

        plt.imshow(spatial_attention[task].mean(0))
        plt.savefig(os.path.join(analysisdir, 'spatial-attention', f'task_spatial_attention_{task}.png'))
        plt.close()
...
if __name__=='__main__':
    ...
    def get_arguments():
        ...
        parser.add_argument('-ds', '--sourcedir', type=str, default='/DataCommon2/jwlee/2023_07_29_ST_Adaptive_Number_Of_Clusters/data')
        parser.add_argument('-dt', '--targetdir', type=str, default='/DataCommon2/jwlee/2023_07_29_ST_Adaptive_Number_Of_Clusters/attention_FC_10/result')
        ...
        parser.add_argument("--epochs_ae", type=int, default=10, help="Epochs number of the autoencoder training", )
