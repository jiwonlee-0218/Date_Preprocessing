...
i=0
with open(save_txt + '/QC_signal_zero_dparsf_JI_list.txt', 'a') as the_file:
    for file in sorted(glob.glob(path1 + '*/ROIs/AAL1_116/noglobal/*.mat')):
        signal = sio.loadmat(file)['ROI']
        i += 1
        if np.count_nonzero(signal.astype('float')) == 0:
            print(os.path.basename(file), 'problem')
